...
class Mamba_FFT(nn.Module):
    def __init__(
        ...
        # add by HJ
        self.fft_conv = nn.Conv1d(self.d_inner*2, self.d_inner*2, 5, 1, 2)
        self.gate_mapping = nn.Linear(self.d_model*128, 3)
        self.adaptive_avg_pool = nn.AdaptiveAvgPool2d((self.d_model, 2048))
        self.elu = nn.ELU()
    ...
    def forward(self, hidden_states, inference_params=None, No_block=-1, SpectralGatingBlocks=None, GateModules=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v2":
                ...
                x_fft_real , x_fft_imag= torch.chunk(x_fft, 2, dim = 1)
                x_fft = torch.complex(x_fft_real, x_fft_imag)
                if No_block == 0:
                    filter_threshold = self.low_freq
                    abs_x_fft = torch.abs(x_fft)
                    high_pass_filter = abs_x_fft < filter_threshold
                    x_fft = x_fft * high_pass_filter
                elif No_block == 1:
                    low_filter_threshold = self.low_freq
                    high_filter_threshold = self.high_freq
                ...
                x_fft_out = torch.fft.ifftn(x_fft, dim = (-2,-1))
